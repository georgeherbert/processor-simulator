...
struct memory_unit
{
    ...
    struct reg *res_stations_ready_memory;    // Pointer to indicate if memory reservation station is ready
};
...
struct memory_unit *memory_init(
    ...
    struct reg *res_stations_ready_memory);           // Initialise memory unit
