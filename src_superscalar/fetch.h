...
struct fetch_unit
{
    ...
    struct reg *reg_insts[ISSUE_WIDTH];     // Pointer to instruction register
    struct reg *reg_inst_pcs[ISSUE_WIDTH];  // Pointer to program counter of instruction in instruction register
    struct reg *reg_npc_preds[ISSUE_WIDTH]; // Pointer to predicted next program counter (for speculative execution)
